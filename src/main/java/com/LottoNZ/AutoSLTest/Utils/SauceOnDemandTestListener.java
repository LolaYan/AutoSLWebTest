...
public class SauceOnDemandTestListener extends TestListenerAdapter {
	...
	/**
	 * Check to see if environment variables that define the Selenium browser to
	 * be used have been set (typically by a Sauce OnDemand CI plugin). If so,
	 * then populate the appropriate system parameter, so that tests can use
	 * these values.
	 *
	 * @param testContext
	...
	/**
	 * @param testResult
	 *            Test result of the test that just failed.
	 */
	@Override
	public void onTestFailure(ITestResult testResult) {
		SauceOnDemandSessionIdProvider sessionIdProvider = (SauceOnDemandSessionIdProvider) testResult
				.getInstance();
	...
	/**
	 * @param result
	 *            Test Result for the test being started
	 */
	@Override
	public void onTestStart(ITestResult result) {
		...
		if (result.getInstance() instanceof SauceOnDemandAuthenticationProvider) {
			...
			SauceOnDemandAuthenticationProvider provider = (SauceOnDemandAuthenticationProvider) result
					.getInstance();
		...
		this.sauceREST = new SauceREST(
				sauceOnDemandAuthentication.getUsername(),
				sauceOnDemandAuthentication.getAccessKey());
	}

	/**
	 * @param testResult
	 *            Test result for the test that just passed.
	 */
	@Override
	...
	private void printOutSessionID(String sessionId, String testName) {
		System.out.println(String.format(
				"SauceOnDemandSessionID=%1$s job-name=%2$s", sessionId,
				testName));
	}
