...
public class TestMethodListener implements IInvokedMethodListener {
	...
	public void afterInvocation(IInvokedMethod method, ITestResult testResult) {
		...
		if (method.isTestMethod()) {
			...
			if (lThrowable.size() > 0) {
				...
				/*
				 * if there is assertion error/exception then add it to
				 * throwable list
				 */
				...
				/*
				 * if there is only one throwable then set it directly to test
				 * result
				 */
				if (size == 1) {
					...
					for (int i = 0; i < size - 1; i++) {
						failureMessage.append("(").append(i + 1).append(")")
								.append(lThrowable.get(i).getClass().getName())
								.append(":")
								.append(lThrowable.get(i).getMessage())
								.append("\n");
						fullStack.append("Failure ").append(i + 1)
								.append(" of ").append(size).append("\n");
						fullStack.append(
								Utils.stackTrace(lThrowable.get(i), false)[1])
								.append("\n");
					}
					...
					failureMessage.append("(").append(size).append(")")
							.append(last.getClass().getName()).append(":")
							.append(last.getMessage()).append("\n\n");

					fullStack.append(last.toString());

					testResult.setThrowable(new Throwable(failureMessage
							.toString() + fullStack.toString()));
	...
	public void beforeInvocation(IInvokedMethod method, ITestResult testResult) {
		...
		if (method.isTestMethod()) {
			...
			TestMethodErrorBuffer.set(new ArrayList<Throwable>()); // each test
																	// method
																	// will have
																	// its own
																	// error
